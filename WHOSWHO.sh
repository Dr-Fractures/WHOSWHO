...
echo "# Netscan session $TS" > "$INDEX_FILE"
...
banner "Scan Plan"
cat <<EOF
This script will step through the following stages. You'll be asked
before each one runs, and shown the exact nmap command first:

  1. Host discovery (ping sweep)          - find live hosts
  2. Fast port scan (top ports)           - quick TCP scan on live hosts
  3. Service/version detection            - -sV on discovered open ports
  4. OS detection                         - -O (best effort, needs root)
  5. Full TCP port scan (optional, slow)  - -p- all 65535 ports
...
if ! confirm "Proceed with scan plan against $TARGET?"; then
...
# Extract live hosts for later stages
...
if [[ -s "$LIVE_HOSTS_FILE" ]]; then
    ...
    color "33" "No live hosts parsed (host discovery may have been skipped or hosts block ping). Falling back to full subnet for remaining stages."
    SCAN_TARGET="$TARGET"
fi
echo
...
# Stage 5: Full TCP port scan (optional - slow)
if confirm "Stage 5 (full 65535-port scan) can take a long time. Include it in the plan?"; then
    run_step "Full TCP port scan (all 65535 ports)" \
        "nmap -p- -T4 -oN '${SESSION_DIR}/tmp_fullports.txt' $SCAN_TARGET" \
        "full_port_scan"
else
    color "33" "Skipping full port scan by choice."
fi
